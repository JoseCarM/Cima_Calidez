...
class Menu extends React.Component {
  constructor(props) {
    ...
    this.interaccionMenu = this.interaccionMenu.bind(this);
  ...
  abrirCabana(event) {
    ...
    let cabana;
    if (
      event.target.id === "menuCab1" ||
      event.target.id === "imgMenuCab1" ||
      event.target.id === "textoMenuCab1"
    ) {
      cabana = 1;
    } else if (
      event.target.id === "menuCab2" ||
      event.target.id === "imgMenuCab2" ||
      event.target.id === "textoMenuCab2"
    ) {
      cabana = 2;
    } else if (
      event.target.id === "menuCab3" ||
      event.target.id === "imgMenuCab3" ||
      event.target.id === "textoMenuCab3"
    ) {
      cabana = 3;
    } else if (
      event.target.id === "menuCab4" ||
      event.target.id === "imgMenuCab4" ||
      event.target.id === "textoMenuCab4"
    ) {
      cabana = 4;
    } else if (
      event.target.id === "menuCab5" ||
      event.target.id === "imgMenuCab5" ||
      event.target.id === "textoMenuCab5"
    ) {
      cabana = 5;
    }
  ...
  render() {
    return (
      <div id="menu">
        <button
          onClick={this.interaccionMenu}
          onBlur={this.interaccionMenu}
          id="botonMenu"
        >
          <p>Menú</p>
        </button>
        <div id="contenedorOpcionesMenu">
          <div
            id={this.state.desplegado ? "menuInicio" : "menuInicioNoVisible"}
            className={
              this.state.desplegado ? "itemsDeMenu" : "itemsDeMenuNoVisibles"
            }
          >
          ...
          <div
            id={
              this.state.desplegado && !this.props.comparar
                ? "menuComparador"
                : "menuComparadorNoVisible"
            }
            className={
              this.state.desplegado && !this.props.comparar
                ? "itemsDeMenu"
                : "itemsDeMenuNoVisibles"
            }
            onMouseDown={this.abrirComparador}
          >
            <img
              src={iconoComparador}
              alt="Comparar precios y disponibilidad"
            />
            <p>Comparar</p>
          </div>
          <div
            onMouseDown={this.abrirCabana}
            id="menuCab1"
            className={
              this.state.desplegado ? "itemsDeMenu" : "itemsDeMenuNoVisibles"
            }
