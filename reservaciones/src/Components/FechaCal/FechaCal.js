...
class FechaCal extends React.Component {
  ...
  defineFechaMaximaDeSalida() {
    if (this.props.fechasOcupadas.length > 0) {
      ...
      this.props.fechasOcupadas.forEach((fechaOcupada) => {
        if (fechaOcupada.getTime() > this.props.fecha.getTime()) {
          if (
            ...
            numeroDeDiasEntreFechas(this.props.fecha, fechaOcupada) <
              numeroDeDiasEntreFechas(this.props.fecha, fechaOcupadaMasCercana)
          ) {
      ...
      this.props.cambioDeEstadoApp({
        fechaOcupadaMasCercanaAFechaDeEntrada: fechaOcupadaMasCercana,
      });
    }
  }
  ...
  seleccionarFecha() {
    ...
    if (
      this.props.fechaDeEntrada !== null &&
      this.props.fechaDeSalida !== null
    ) {
      ...
      this.props.fechasOcupadas.some(
        (fechaOcupada) => fechaOcupada.getTime() === this.props.fecha.getTime()
      ) === false
    ) {
      ...
      if (this.props.fechaDeEntrada === null) {
        ...
        if (this.props.fecha < this.props.fechaDeEntrada) {
          ...
          this.props.fecha.getTime() === this.props.fechaDeEntrada.getTime()
        ) {
          this.props.cambioDeEstadoApp({
            fechaDeEntrada: null,
            fechaOcupadaMasCercanaAFechaDeEntrada: null,
          ...
          if (
            this.props.fecha <= this.props.fechaOcupadaMasCercanaAFechaDeEntrada
          ) {
      ...
      if (
        this.props.fecha.getTime() ===
          this.props.fechaOcupadaMasCercanaAFechaDeEntrada.getTime() &&
        this.props.fechaDeEntrada !== null &&
        this.props.fechaDeSalida === null
      ) {
        this.props.cambioDeEstadoApp({ fechaDeSalida: this.props.fecha });
    ...
    if (!this.props.fechaOcupadaMasCercanaAFechaDeEntrada) {
      if (
        this.props.fechaDeEntrada !== null &&
        this.props.fechaDeSalida === null &&
        this.props.fecha > this.props.fechaDeEntrada
      ) {
        this.props.cambioDeEstadoApp({ fechaDeSalida: this.props.fecha });
        document
          .getElementById("presentacionDeHuespedes")
  ...
  clasePorFecha() {
    ...
    for (let i = 0; i < this.props.fechasOcupadas.length; i++) {
      ...
      if (
        this.props.fechasOcupadas[i].getTime() === this.props.fecha.getTime()
      ) {
        fechaReservada = true;
      }
    }
    ...
    if (this.props.fecha < fechaActual) {
      ...
      return this.props.fecha >= this.props.fechaDeEntrada &&
        this.props.fecha <= this.props.fechaDeSalida
        ? "fechasSeleccionadas"
  ...
  IdEntradaOSalida() {
    ...
    let fechaDeEntrada =
      this.props.fechaDeEntrada === null
        ? 0
        : this.props.fechaDeEntrada.getTime();
    let fechaDeSalida =
      this.props.fechaDeSalida === null
        ? 0
        : this.props.fechaDeSalida.getTime();
  ...
  render() {
    return (
      <div
        onClick={this.seleccionarFecha}
        className={this.clasePorFecha()}
        id={this.IdEntradaOSalida()}
      >
