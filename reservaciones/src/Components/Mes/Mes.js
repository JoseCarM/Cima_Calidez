  ...
  renderizarDias() {
    ...
    const filaSemana = (numeroDeFila) => {
      ...
      for (let i = 1; i <= 7; i++) {
        if (
          diasTranscurridos + i <=
          diasDelMes(anioActual, mesActualNumerico) + primerDia
        ) {
          if (i <= primerDia && numeroDeFila === 0) {
            // Introduce los primero td en blanco si es que existen
          ...
          // Introduce los ultimos td en blanco si es que existen
          fila.push(<td key={`td${i}`}></td>);
        }
      }
      return fila;
    ...
    return (
      <div className="meses">
        <table>
          <thead>
            <tr>
              <th id="mesYAnio" colSpan="7">
                {this.mesActual()} {this.props.anio}
              </th>
            </tr>
          ...
          <tbody>{this.renderizarDias()}</tbody>
        </table>
      </div>
    );
  }
