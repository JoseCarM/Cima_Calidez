...
class ListaDeResultados extends React.Component {
    ...
    renderResultados() {
        ...
        let huespedes = this.props.huespedes.numeroDeAdultos + this.props.huespedes.numeroDeNinos;
        let opciones = [];
        let precio;
        if(!this.props.busquedaCombinada){
            resultados.forEach(resultado => {
                if(huespedes <= this.props.maximoDeHuespedes[`cabana${resultado}`]){
                    ...
                    opciones.push(<Resultado cabana={resultado} key={resultado} precio={precio} cambioDeEstadoApp={this.props.cambioDeEstadoApp}/>);
                }
            });
            return opciones;
        } else if(this.props.busquedaCombinada){
            const sumar = (propiedad) => (acumulador, valorActual) => acumulador + propiedad[`cabana${valorActual}`];
            resultados = combinador(resultados);
            resultados = resultados.filter(combinacion => combinacion.reduce(sumar(this.props.maximoDeHuespedes), 0) >= huespedes);
            let numeroDeOpcion = 1;
            resultados.forEach(combinacion => {
                precio = combinacion.reduce(sumar(this.props.preciosBase), 0);
                precio += ((huespedes - combinacion.reduce(sumar(this.props.minimoDeHuespedes), 0)) < 0)? 0 : (huespedes - combinacion.reduce(sumar(this.props.minimoDeHuespedes), 0)) * 250;
                precio += this.props.huespedes.numeroDeMascotas * 150;
                precio *= this.props.numeroDeNoches;
                opciones.push(<div className='opcionDeCombinacion' key={numeroDeOpcion}><p>Opción {numeroDeOpcion}</p><ResultadoCombinado combinacion={combinacion} key={combinacion} precio={precio} cambioDeEstadoApp={this.props.cambioDeEstadoApp}/></div>);
                numeroDeOpcion++;
            })
            return opciones;
        }
    ...
    render(){
        return(
            <div id={(this.props.busquedaCombinada)? 'listaDeResultadosColumna' : 'listaDeResultadosFila'}>
                {this.mensaje()}
