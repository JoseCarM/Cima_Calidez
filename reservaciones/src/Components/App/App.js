...
class App extends React.Component {
  constructor(props) {
    ...
    this.cambiarNumeroMaximoDeHuespedes = this.cambiarNumeroMaximoDeHuespedes.bind(
      this
    );
  ...
  cambiarNumeroMaximoDeHuespedes() {
    if (this.state.cabana !== null) {
      ...
      huespedes.numeroMaximoDeHuespedes = this.state.maximoDeHuespedes[
  ...
  async buscaFechasOcupadas() {
    ...
    if (this.state.cabana !== null) {
      ...
      reservaciones.forEach((reservacion) => {
        ...
        for (
          let i = 0;
          i < numeroDeDiasEntreFechas(fechaDeEntrada, fechaDeSalida) - 1;
          i++
        ) {
  ...
  render() {
    if (this.state.cargando) {
      ...
      if (this.state.seReservoEnElMomento) {
        return (
          <div id="divCentro">
            <h1>
              Tenemos malas noticias, parece que alguien reservo la opción que
              selecciono hace un momento.
            </h1>
            <button
              id="botonVolverABuscar"
              onClick={() => this.setState({ seReservoEnElMomento: false })}
            >
              Volver a buscar
            </button>
          </div>
      ...
      if (this.state.listoParaReservar) {
        return (
          <PanelDeReservacion
            cabanasARentar={this.state.cabanasARentar}
            fechaDeEntrada={this.state.fechaDeEntrada}
            fechaDeSalida={this.state.fechaDeSalida}
            huespedes={this.state.huespedes}
            costoTotal={this.state.costoTotal}
            cambioDeEstadoApp={this.cambioDeEstadoApp}
            buscaFechasOcupadas={this.buscaFechasOcupadas}
          />
        );
      } else {
          ...
          return (
            <div id="app">
              ...
              <Comparador
                ...
                fechaDeSalida={this.state.fechaDeSalida}
                huespedes={this.state.huespedes}
                preciosBase={this.state.preciosBase}
                minimoDeHuespedes={this.state.minimoDeHuespedes}
                maximoDeHuespedes={this.state.maximoDeHuespedes}
                fechasOcupadas={this.state.fechasOcupadas}
                cambioDeEstadoApp={this.cambioDeEstadoApp}
              />
            </div>
          );
          ...
          return (
            <div id="app">
              ...
              <Cabana
                cabana={this.state.cabana}
                fechaDeEntrada={this.state.fechaDeEntrada}
                fechaOcupadaMasCercanaAFechaDeEntrada={
                  this.state.fechaOcupadaMasCercanaAFechaDeEntrada
                }
                fechaDeSalida={this.state.fechaDeSalida}
                huespedes={this.state.huespedes}
                preciosBase={this.state.preciosBase}
