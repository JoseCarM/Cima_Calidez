...
class CantidadDeHuespedes extends React.Component {
  ...
  cambiarNumeroDeHuespedes(tipo, cantidad, cambio) {
    ...
    switch (tipo) {
      case "Adultos":
        if (
          cantidad + huespedes.numeroDeNinos <
            huespedes.numeroMaximoDeHuespedes &&
          cambio === "aumenta"
        ) {
          cantidad++;
      ...
      default:
        let err = new Error(
          "Debe haber un Bug en cambiarNumeroDeHuespedes de CantidadDeHuespedes"
        );
        console.log(err);
    }
    this.props.cambioDeEstadoApp(huespedes);
  }
      ...
      <tr className="tipoDeHuesped">
        ...
        <td>
          <button
            ...
            onClick={() =>
              this.cambiarNumeroDeHuespedes(tipo, cantidad, "disminuye")
            }
          >
            -
          </button>
          ...
          <button
            className="botonesHuespedes"
            onClick={() =>
              this.cambiarNumeroDeHuespedes(tipo, cantidad, "aumenta")
            }
          >
            +
          </button>
  ...
  render() {
    return (
      <div id="presentacionDeHuespedes">
        <h1>Indique la cantidad de Huespedes</h1>
        <table>
          <tbody>
            {this.renderizaOpciones(
              "Adultos",
            ...
            {this.renderizaOpciones(
              "Bebes",
              this.props.huespedes.numeroDeBebes
            )}
            {this.renderizaOpciones(
              "Mascotas",
